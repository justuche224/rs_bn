...
export class WalletController {
    ...
    static async uploadSystemWallet(c) {
        try {
            ...
            const qrCodeFile = formData.get("wallQRCode");
            if (!currency || !address || !qrCodeFile) {
                return c.json({
                    error: "Missing required fields: currency, address, and QR code are required",
                }, 400);
            }
            const result = await walletService.uploadSystemWallet(currency, address, qrCodeFile);
    ...
    static async serveSystemWalletQRCode(c) {
        try {
    ...
    static async getSystemWalletInfo(c) {
        try {
    ...
    static async getAllSystemWallets(c) {
        try {
    ...
    static async updateSystemWallet(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            ...
            const result = await walletService.updateSystemWallet(walletId, currency, address, qrCodeFile);
    ...
    static async deleteSystemWallet(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
    ...
    // User Wallet Methods
    static async createUserWallet(c) {
        try {
            const user = c.get("user");
            if (!user) {
            ...
            const result = await walletService.createUserWallet(user.id, currency, address);
            return c.json({ success: true, wallet: result }, 201);
        }
        catch (error) {
    ...
    static async deleteUserWallet(c) {
