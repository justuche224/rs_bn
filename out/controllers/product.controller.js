...
export class ProductController {
    ...
    static async deleteCategories(c) {
        try {
            const { categoryIds } = await c.req.json();
            if (!Array.isArray(categoryIds) || categoryIds.length === 0) {
                return c.json({ error: "categoryIds must be a non-empty array" }, 400);
            }
    ...
    static async createProduct(c) {
        try {
            ...
            if (!name || !price || !description || !categoryId || !imageFile) {
                return c.json({ error: "All fields (name, price, description, categoryId, image) are required" }, 400);
            }
    ...
    static async getProductImage(c) {
        try {
            ...
            return new Response(error instanceof Error ? error.message : "Image not found", { status: 404 });
        }
    }
}
