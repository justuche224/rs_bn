...
export class UserController {
    ...
    static async deleteUsers(c) {
        try {
            ...
            if (!Array.isArray(userIds) || userIds.length === 0) {
                return c.json({ error: "Invalid request: userIds must be a non-empty array" }, 400);
            }
    ...
    static async updateProfilePicture(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
    ...
    static async getProfilePicture(c) {
        try {
            const user = c.get("user");
            if (!user) {
                return c.json({ error: "Unauthorized: No user found" }, 401);
            ...
            return new Response(error instanceof Error ? error.message : "File not found", { status: error instanceof Error && error.message.includes("Unauthorized") ? 403 : 404 });
        }
    }
}
