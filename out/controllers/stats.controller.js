...
export class StatsController {
    static getInvestmentStats(arg0, getInvestmentStats) {
        throw new Error("Method not implemented.");
    }
    static async getTransactionStats(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
            }
            const [totalDeposits, pendingDeposits, approvedDepositsAmount, totalWithdrawals, pendingWithdrawals, approvedWithdrawalsAmount,] = await Promise.all([
    ...
    static async getProductStats(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
    ...
    static async getTransactionHistory(c) {
        try {
            const user = c.get("user");
            if (!user || user.role !== "ADMIN") {
                return c.json({ error: "Unauthorized: Admin access required" }, 403);
    ...
    static async getUserTransactionHistory(c) {
        try {
            const user = c.get("user");
            ...
            if (!user || (user.id !== userId && user.role !== "ADMIN")) {
                return c.json({
                    error: "Unauthorized: Can only view your own transactions or must be admin",
                }, 403);
            }
            const [userDeposits, userWithdrawals, userInvestments] = await Promise.all([
    ...
    static async getUserReferralStats(c) {
        try {
            const user = c.get("user");
            const { userId } = c.req.param();
            ...
            if (!user || (user.id !== userId && user.role !== "ADMIN")) {
                return c.json({
                    error: "Unauthorized: Can only view your own referral stats or must be admin",
                }, 403);
            }
    ...
    static async getUserSimpleInvestmentStats(c) {
        try {
            const user = c.get("user");
            const { userId } = c.req.param();
            // Check if user is authorized to view these stats
            if (!user || (user.id !== userId && user.role !== "ADMIN")) {
                return c.json({
                    error: "Unauthorized: Can only view your own investment stats or must be admin",
                }, 403);
            }
            const stats = await investmentService.getUserSimpleInvestmentStats(userId);
            return c.json(stats, 200);
        }
        catch (error) {
