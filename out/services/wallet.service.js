...
export class WalletService {
    ...
    async uploadSystemWallet(currency, address, qrCodeFile) {
        ...
        const existingWallet = await db
            .select()
            .from(systemWallet)
            .where(and(eq(systemWallet.currency, currency), eq(systemWallet.isActive, true)));
    ...
    async getSystemWalletQRCode(filename) {
    ...
    async getSystemWalletInfo(currency) {
        const walletRecord = await db
            .select()
            .from(systemWallet)
            .where(and(eq(systemWallet.currency, currency), eq(systemWallet.isActive, true)))
    ...
    async updateSystemWallet(walletId, currency, address, qrCodeFile) {
        const existingWallet = await db
            .select()
            .from(systemWallet)
    ...
    async deleteSystemWallet(walletId) {
        const existingWallet = await db
            .select()
            .from(systemWallet)
            .where(eq(systemWallet.id, walletId))
    ...
    // User Wallet Methods
    async createUserWallet(userId, currency, address) {
        // Check if user already has a wallet with this currency
        const existingWallet = await db
            .select()
            .from(userWallet)
            .where(and(eq(userWallet.userId, userId), eq(userWallet.currency, currency)));
        if (existingWallet.length > 0) {
    ...
    async deleteUserWallet(walletId, userId, isAdmin) {
        const existingWallet = await db
            .select()
            .from(userWallet)
