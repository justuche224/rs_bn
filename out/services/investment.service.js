...
export class InvestmentService {
    ...
    async createInvestment(userId, planId, currency) {
        // 1. Get plan details
        ...
        if (userRecord && userRecord.email) {
            await mailService.sendInvestmentNotification(userRecord.email, plan.price.toString(), plan.type, currency, "ACTIVE");
        }
    ...
    async updateInvestmentStatus(investmentId, status) {
        ...
        if (userRecord && userRecord.email && plan) {
            await mailService.sendInvestmentNotification(userRecord.email, investment.amount.toString(), plan.type, investment.currency, status);
        }
        return { success: true };
    }
