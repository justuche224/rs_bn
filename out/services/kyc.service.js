...
export class KYCService {
    ...
    async uploadKYCImages(userId, frontFile, backFile, selfieFile, documentType) {
        const files = [
    ...
    async getKYCImage(filename, userId, isAdmin) {
        const kycRecord = await db
            .select()
            .from(kyc)
            .where(and(or(eq(kyc.frontImage, filename), eq(kyc.backImage, filename), eq(kyc.selfieImage, filename))))
            .limit(1);
    ...
    async updateKYCStatus(kycId, status, rejectionReason) {
