...
export async function uploadKYCImage(c: Context) {
  try {
...
export async function serveKYCImage(c: Context) {
  try {
...
export async function checkKYCStatus(c: Context) {
...
export async function updateKYCStatus(c: Context) {
  try {
...
export async function getKYCInfo(c: Context) {
  try {
    const user = c.get("user");
    if (!user) {
      return c.json({ error: "Unauthorized: No user found" }, 401);
...
export const getAllKYC = async (c: Context) => {
  try {
    const userInfo = c.get("user");
    if (!userInfo) {
      return c.json({ error: "Unauthorized: No user found" }, 401);
