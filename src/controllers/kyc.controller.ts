...
export class KYCController {
  static async uploadKYCImage(c: Context) {
    try {
      ...
      if (!frontFile || !backFile || !selfieFile || !documentType || !userId) {
        return c.json(
          {
            error:
              "Missing required fields: front, back, selfie images, documentType, and userId are required",
          },
          400
        );
      }
      ...
      const result = await kycService.uploadKYCImages(
        userId,
        frontFile,
        backFile,
        selfieFile,
  ...
  static async serveKYCImage(c: Context) {
    try {
      ...
      const fileBuffer = await kycService.getKYCImage(
        filename,
        userId,
        isAdmin
      );
  ...
  static async checkKYCStatus(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
        return c.json({ error: "Unauthorized: No user found" }, 401);
  ...
  static async updateKYCStatus(c: Context) {
    try {
      const user = c.get("user");
      ...
      if (!kycId || !status) {
        return c.json(
          { error: "Missing required fields: kycId and status are required" },
          400
        );
      }
  ...
  static async getKYCInfo(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
        return c.json({ error: "Unauthorized: No user found" }, 401);
  ...
  static async getAllKYC(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Unauthorized: Admin access required" }, 403);
