...
export class InvestmentController {
  ...
  static async updateInvestmentStatus(c: Context) {
    try {
      const user = c.get("user");
  ...
  static async getInvestmentById(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
        return c.json({ error: "Unauthorized: No user found" }, 401);
      ...
      try {
        if (user.role === "ADMIN") {
          investment = await investmentService.getInvestmentById(
            idValidation.data
          );
        } else {
          investment = await investmentService.getInvestmentById(
  ...
  static async getUserInvestments(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
        return c.json({ error: "Unauthorized: No user found" }, 401);
  ...
  static async getAllInvestments(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Forbidden: Admin access required" }, 403);
