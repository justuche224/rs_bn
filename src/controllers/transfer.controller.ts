...
export class TransferController {
  static async getAllTransfers(c: Context) {
    try {
  ...
  static async getUserTransfers(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
  ...
  static async createInternalTransfer(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
        return c.json({ error: "Unauthorized: No user found" }, 401);
      ...
      if (!fromCurrency || !toCurrency || !amount) {
        return c.json(
          {
            error:
              "Missing or invalid fields: fromCurrency, toCurrency, and amount are required",
          },
          400
        );
      }
  ...
  static async createInterUserTransfer(c: Context) {
    try {
      const user = c.get("user");
      if (!user) {
        return c.json({ error: "Unauthorized: No user found" }, 401);
      ...
      if (!recipientEmail || !currency || !amount) {
        return c.json(
          {
            error:
              "Missing required fields: recipientEmail, currency, and amount are required",
          },
          400
        );
      }

      const result = await transferService.createInterUserTransfer(
        user.id,
        recipientEmail,
        currency,
        amount,
  ...
  static async approveTransfer(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Unauthorized: Admin access required" }, 403);
  ...
  static async rejectTransfer(c: Context) {
    try {
      const user = c.get("user");
      if (!user || user.role !== "ADMIN") {
        return c.json({ error: "Unauthorized: Admin access required" }, 403);
